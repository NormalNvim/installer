...
nvim -c ':NvimUpdatePlugins' -c ':MasonInstall ansible-language-server angular-language-server asm-lsp asmfmt autopep8 autoflake bash-debug-adapter bash-language-server codelldb clangd cmakelint csharpier debugpy delve docker-compose-language-service dockerfile-language-server elixir-ls eslint-lsp fantomas firefox-debug-adapter fsautocomplete golangci-lint golangci-lint-langserver gopls google-java-format helm-ls java-test jedi-language-server json-lsp kotlin-debug-adapter kotlin-language-server ktlint lua-language-server markuplint matlab-language-server neocmakelsp netcoredbg omnisharp perlnavigator php-debug-adapter phpactor php-cs-fixer phpstan ruby-lsp rubocop ruff ruff-lsp rust-analyzer selene shellcheck shfmt svelte-language-server stylua taplo typescript-language-server yaml-language-server zls' -c ':TSInstall all' 2>&1
